...
function unzip(file, callback) {
    ...
    zEntries.forEach(function(zipFile) {
        ...
        if (zipFile.isDirectory || (name.indexOf(search_basename) < 0) || name[0] === '.' || basename[0] === '.') {
            ...
            try {
                // We implement here the writting based on adm-zip writeFileTo
                // because extractEntryTo doesn't allow to change the destination name (just the folder)
                ...
                var fd;
                try {
                    fd = fs.openSync(dest, 'w', 438); // 0666
                } catch (e) {
                    fs.chmodSync(dest, 438);
                    fd = fs.openSync(dest, 'w', 438);
                }
                if (fd) {
                    try {
