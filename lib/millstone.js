    ...
    existsAsync(options.filepath, function(exists) {
        if (exists) {
            ...
            if (path.extname(options.filepath) == '.zip') {
                try {
                  var zf = new AdmZip(options.filepath);
                  if (zf.getEntries().length < 1) {
...
// Checks if the destination is writtable and creates the destination folder if it doesn't exist
// callback(error, writtable)
function checkDestination (destination, overwrite, callback) {
    fs.exists(destination, exists => {
        ...
        fs.stat(destination, (err, stat) => {
            if (!err && exists && stat.isDirectory()) {
                return callback(null, false);
            }
            ...
            fs.exists(folder, folderExists => {
                if (folderExists) {
                    return callback(null, true); // Ready to write
                }
                fs.mkdir(folder, err => callback(err, true));
            });
        });
    });
}

// Based on https://github.com/cthackers/adm-zip/blob/f66dc607e0c6f31affa0ff75461537f77296b4ef/util/utils.js#L136
// to allow writting the contents of the zipfile to a fixed destination, as extractEntryTo doesn't allow
// to change the file name, only the basename
// callback(error, written)
function writeFileToAsync (destination, content, overwrite, callback) {
    checkDestination(destination, overwrite, (err, ready) => {
        if (err || !ready) {
            return callback(err, ready);
        }
        fs.open(destination, 'w', 438, (err, fd) => {
            if (err || !fd) {
                return callback(err, false);
            }

            fs.write(fd, content, 0, content.length, 0, (err) => {
                if (err) {
                    fs.close(fd, () => { return callback(err, false); });
                }
...
function unzip(file, callback) {
    ...
    const zEntries = zf.getEntries();

    if (zEntries.length < 1) {
        return callback(new Error("could not find any valid data in zip archive: '" + file + "'"));
    }

    var remain = zEntries.length;
    var ds_files = _(zEntries.map(ze => ze.entryName)).chain()
    ...
    zEntries.forEach(function(zipFile) {
        // Skip directories, hiddens.
        var name = zipFile.entryName;
        var basename = path.basename(name);
        if (zipFile.isDirectory || (name.indexOf(search_basename) < 0) || name[0] === '.' || basename[0] === '.') {
            remain--;
            ...
            const content = zipFile.getData();
            const overwrite = true;
            writeFileToAsync(dest, content, overwrite, (err, written) => {
                if (err) {
                    return callback(err);
                }
                if (written) {
